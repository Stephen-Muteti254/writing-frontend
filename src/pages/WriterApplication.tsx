...
const WriterApplication = () => {
  ...
  const handleFileUpload = (field: string, files: FileList | null, multiple = false) => {
    ...
    if (multiple) {
      ...
      setFormData((prev) => ({
        ...prev,
        [field]: [...(prev[field as keyof typeof prev] as File[]), ...uploadedFiles],
      }));
    } else {
      setFormData((prev) => ({
        ...prev,
    ...
    if (currentStep === 1) {
      ...
      if (Object.keys(formData.proficiencyAnswers).length !== 30) {
        toast({
          ...
          description: "Please answer all 30 questions.",
          variant: "destructive",
        });
        return;
      }
      ...
      if (!formData.selectedPrompt || !formData.promptResponse) {
        toast({
          ...
          description: "Please select a prompt and provide your response.",
          variant: "destructive",
        });
        return;
      }
      ...
      if (wordCount < 100 || wordCount > 150) {
        toast({
          title: "Word count requirement",
          description: "Your response must be between 100-150 words.",
          variant: "destructive",
        });
        return;
      }
      ...
      if (!formData.selectedEssayTopic || !formData.essayFile) {
        toast({
          ...
          description: "Please select a topic and upload your essay.",
          variant: "destructive",
        });
        return;
      }
      ...
      if (formData.degreeCertificates.length === 0) {
        toast({
          ...
          description: "Please upload your degree certificate.",
          variant: "destructive",
        });
        return;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formDataToSend.append("proficiencyAnswers", JSON.stringify(formData.proficiencyAnswers));
      formDataToSend.append("selectedPrompt", formData.selectedPrompt);
      formDataToSend.append("promptResponse", formData.promptResponse);
      formDataToSend.append("selectedEssayTopic", formData.selectedEssayTopic);

      // Append files if present
      if (formData.essayFile) {
        formDataToSend.append("essayFile", formData.essayFile);
      }
      if (formData.cvFile) {
        formDataToSend.append("cvFile", formData.cvFile);
      }
      formData.workSamples.forEach((file) => formDataToSend.append("workSamples", file));
      formData.degreeCertificates.forEach((file) => formDataToSend.append("degreeCertificates", file));

      const response = await api.post("/applications/apply-writer", formDataToSend, {
      ...
      if (data?.success) {
        ...
        toast({
          ...
          description: data?.error?.message || "Unable to submit your application. Please try again.",
          variant: "destructive",
        });
      }
    } catch (err: any) {
